class Node {
  constructor ({ x, y, endDistance = 0, startDistance = 9999 }) {
    this.x = x
    this.y = y
 ...
 * @returns {Node[][]}
 */
function buildNodes (matrix, endNode, startNode) {
  let nodes = []
  ...
  for (let x = 0; x < matrix.length; x++) {
    ...
    for (let y = 0; y < row.length; y++) {
      if (row[y] > 0) {
        ...
        // const startDistance = getDistance({ x, y }, { x: startNode[0], y: startNode[1] })
...
function solveMaze (matrix, start, end) {
  let nodes = buildNodes(matrix, end, start)
  ...
  while (queue.length) {
    ...
    for (let n of node.nextNodes) {
      if (n.passed === false) {
        ...
        n.startDistance = (node.startDistance + 1) * 1 // 更新与起点的距离
